...
# completa_json = ['aams', '#nome#nonserve', 'alias', 'barcode', 'barcode_stecca', 'unita', 'categoria', 'vendita', '€/kg conv', 'prezzo', 'stecca#nonserve']
...
dfj_vendita = df_json["vendita"].values.tolist()
...
for sigaretta in elenco['lista_sigarette']:
  for (aams, alias, barcode, barcode_stecca, min_kgc, categoria, vendita) in zip(dfj_aams, dfj_alias1, dfj_barcode, dfj_barcode_stecca, dfj_min_kgc, dfj_categoria, dfj_vendita):
    if sigaretta["codice_aams"] == aams:
      ...
      sigaretta["vendita"] = vendita if not pd.isna(vendita) else ""
